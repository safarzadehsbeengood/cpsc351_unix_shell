...
struct {
  char **argv;  //array of arguments example (ls -l => argv[0] = ls, argv[1] = -l, argv[2] = NULL)
...
struct {
  Command **commands; //holds multiple commands: example: for ls | grep .c -> commands[0] = {"ls", NULL},commands[1] = {"grep", ".c", NULL}
...
Command *rsh_parse_cmd(char *cmd_str) {
  ...
  token = strtok_r(cmd_str, RSH_TOK_DELIM, &saveptr_cmd); //splits cmd_str into tokens
  while (token != NULL) {
    //finds > or >> and stores the filename to write or append to
    if (strcmp(token, ">") == 0 || strcmp(token, ">>") == 0) { //if ">" or ">>" is found in token
    ...
    token = strtok_r(NULL, RSH_TOK_DELIM, &saveptr_cmd); //incroments the next thing in cmd_str based off RSH_TOK_DELIM
  }
